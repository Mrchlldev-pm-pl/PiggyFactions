...
class InviteSubCommand extends FactionSubCommand
{
    ...
    {
        $target = PiggyFactions::getInstance()->getServer()->getPlayerExact($args["name"]);
        if (!$target instanceof Player) {
            $member->sendMessage("commands.invalid-player", ["{PLAYER}" => $args["name"]]);
            return;
        }
        $targetFaction = PiggyFactions::getInstance()->getPlayerManager()->getPlayerFaction($target->getUniqueId());
        ...
        if (count($faction->getMembers()) >= ($maxPlayers = PiggyFactions::getInstance()->getConfig()->getNested("factions.max-players", -1)) && $maxPlayers !== -1 && !$member->isInAdminMode()) {
        ...
        PiggyFactions::getInstance()->getLanguageManager()->sendMessage($target, "commands.invite.invited", ["{FACTION}" => $faction->getName()]);
    }
