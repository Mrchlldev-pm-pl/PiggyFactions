...
class NameSubCommand extends FactionSubCommand
{
    ...
    {
        if (PiggyFactions::getInstance()->getFactionsManager()->getFactionByName($args["name"]) !== null) {
            $member->sendMessage("commands.create.name-taken", ["{NAME}" => $args["name"]]);
            return;
        }
        if (PiggyFactions::getInstance()->getConfig()->getNested("factions.enforce-alphanumeric-names", false) && !ctype_alnum($args["name"])) {
            $member->sendMessage("commands.create.alphanumeric-only", ["{NAME}" => $args["name"]]);
            return;
        }
        if (in_array(strtolower($args["name"]), PiggyFactions::getInstance()->getConfig()->getNested("factions.blacklisted-names", []))) {
            $member->sendMessage("commands.create.name-blacklisted", ["{NAME}" => $args["name"]]);
            return;
        }
        if (strlen($args["name"]) > PiggyFactions::getInstance()->getConfig()->getNested("factions.max-name-length", 16)) {
